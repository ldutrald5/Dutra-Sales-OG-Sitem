(function attachAiService(root, factory) {
  ...
  function localResponse(request) {
    ...
    const knowledgeIds=(request.knowledge||[]).map(item=>item.id).filter(Boolean);
    const hasProtected=(request.knowledge||[]).some(item=>item.externalSafe===false);
    const safeInput=hasProtected ? input.replace(/(?:\+?20%|2\s*[%–-]\s*8%|18\s*[–-]\s*20\s*meses|8\s*[–-]\s*10\s*anos)/gi,'um cenário que precisa ser validado com dados da operação') : input;
    const common = { summary: safeInput || `Ação comercial para ${company}.`, recommendedResponse:'', question:'', objective:'', suggestedNextAction:'', crmSuggestion:null, knowledge_ids_used:knowledgeIds, source:'safe_local_fallback' };
    const responses = {
      ...
      personalize_message:{recommendedResponse:safeInput || `Olá, ${contact}! Posso falar rapidamente sobre a operação da ${company}?`,objective:'Personalizar sem criar fatos.',suggestedNextAction:hasProtected?'Alegações condicionais foram qualificadas; revise antes de enviar.':'Revisar antes de enviar.'},

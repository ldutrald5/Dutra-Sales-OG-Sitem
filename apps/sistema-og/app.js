...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function setSyncStatus(label, mode = 'idle') {
    ...
    badge.title = mode === 'offline' || mode === 'busy'
      ? 'Toque para tentar sincronizar agora'
      : 'Estado da sincronização — toque para atualizar';
  }

  /** Sincronização offline: localStorage sempre; outbox + Background Sync quando a rede volta. */
  async function syncNow(reason = 'manual') {
    if (!navigator.onLine) {
      await queueStateForBackgroundSync();
      setSyncStatus('Modo offline — salvo neste aparelho', 'offline');
      return false;
    }
    setSyncStatus('Sincronizando…', 'busy');
    try {
      const flushed = await flushSyncOutboxFromPage();
      if (!flushed) await pushStateWithMerge(state.leads, state.history);
  ...
  async function writeSyncOutbox(payload) {
    const db = await openSyncDb();
    return new Promise((resolve, reject) => {
      const tx = db.transaction(SYNC_STORE, 'readwrite');
      tx.objectStore(SYNC_STORE).put(payload, 'state');
  ...
  async function queueStateForBackgroundSync() {
    const token = cloudAccessToken();
    const payload = {
      url: '/api/state',
      method: 'PUT',
    ...
    await writeSyncOutbox(payload);
    const registered = await registerBackgroundSync();
    if (registered) {
      setSyncStatus('Fila offline — sincroniza ao reconectar', 'busy');
    } else {
      setSyncStatus('Salvo neste aparelho — aguardando rede', 'offline');
    }
  }

  /**
   * Push com merge nível 2:
   * 1) GET estado remoto
   * 2) merge campo a campo com o local
   * 3) PUT com revision do servidor
   * 4) se 409, mescla de novo e tenta até 3 vezes
   */
  async function pushStateWithMerge(localLeads, localHistory, opts = {}) {
    const maxAttempts = opts.maxAttempts || 3;
    let attempt = 0;
    let workingLeads = localLeads;
    let workingHistory = localHistory;
    let baseRevision = serverRevision;

    while (attempt < maxAttempts) {
      attempt += 1;
      try {
        const getRes = await apiFetch('/api/state', { cache: 'no-store' });
        if (getRes.ok) {
          const remote = await getRes.json();
          baseRevision = Number(remote.revision || 0);
          workingLeads = mergeLeadLists(workingLeads, remote.leads || []);
          workingHistory = mergeHistoryLists(workingHistory, remote.history || []);
        }
      } catch (_) {
        /* sem GET — tenta PUT mesmo assim */
      }

      const putRes = await apiFetch('/api/state', {
        method: 'PUT',
        body: JSON.stringify({
          leads: workingLeads,
      ...
      if (putRes.status === 409) {
        const conflict = await putRes.json().catch(() => ({}));
        baseRevision = Number(conflict.revision || baseRevision);
        workingLeads = mergeLeadLists(workingLeads, conflict.leads || []);
        workingHistory = mergeHistoryLists(workingHistory, conflict.history || []);
        setSyncStatus('Mesclando alterações…', 'busy');
        continue;
      }

      if (!putRes.ok) throw new Error('Servidor indisponível');

      const saved = await putRes.json();
      serverRevision = Number(saved.revision || baseRevision + 1);
      applyMergedState(saved.leads || workingLeads, saved.history || workingHistory);
      return saved;
    }
    throw new Error('Conflito persistente após várias tentativas');
  }

  /** Envia o outbox agora (online event / fallback sem Background Sync API). */
  async function flushSyncOutboxFromPage() {
    const pending = await readSyncOutbox();
    if (!pending?.body) return false;
    try {
      const localLeads = pending.body.leads || state.leads;
      const localHistory = pending.body.history || state.history;
      await pushStateWithMerge(localLeads, localHistory);
      await clearSyncOutbox();
      setSyncStatus('Sincronizado', 'ok');
      renderDayDashboard();
      if (state.currentTab === 'crm') renderCrmModule();
  ...
  function scheduleServerSync() {
    ...
    serverSyncTimer = setTimeout(async () => {
      try {
        ...
        await pushStateWithMerge(state.leads, state.history);
        await clearSyncOutbox();
        setSyncStatus('Sincronizado', 'ok');
      } catch {
        try {
  ...
  async function loadSharedState() {
    try {
      // Se há fila pendente, descarrega com merge antes de consolidar
      await flushSyncOutboxFromPage();
      ...
      if (serverHasData && browserHasData) {
        // Merge em vez de substituir — preserva edições locais recentes
        const mergedLeads = mergeLeadLists(state.leads, shared.leads || []);
        const mergedHistory = mergeHistoryLists(state.history, shared.history || []);
        applyMergedState(mergedLeads, mergedHistory);
        renderDayDashboard();
        if (state.currentTab === 'crm') renderCrmModule();
        if (state.currentTab === 'historico') renderHistory();
        // Empurra resultado mesclado se local tinha algo que o servidor não tinha
        scheduleServerSync();
      } else if (serverHasData) {
        applyMergedState(shared.leads || [], shared.history || []);
        renderDayDashboard();
        if (state.currentTab === 'crm') renderCrmModule();
        if (state.currentTab === 'historico') renderHistory();
      ...
      const pending = await readSyncOutbox();
      if (pending) setSyncStatus('Fila offline — sincroniza ao reconectar', 'busy');
      else setSyncStatus('Salvo neste aparelho', 'offline');
    }
  }
  ...
  function switchTab(tabId) {
    ...
    tabs.forEach(t => {
      if (t.getAttribute('data-tab') === tabId) {
        ...
        t.classList.add('is-active', 'bg-amber-500', 'text-slate-950', 'font-bold', 'shadow-md');
        t.classList.remove('text-slate-400', 'hover:text-slate-200', 'hover:bg-slate-800/60');
      } else {
        t.removeAttribute('aria-current');
        t.classList.remove('is-active', 'bg-amber-500', 'text-slate-950', 'font-bold', 'shadow-md');
...
Pode me passar o valor e o prazo de entrega, por favor?`;
  ...
  function normalizeLead(lead) {
    return {
      ...
      updatedAt: typeof lead.updatedAt === 'string' && lead.updatedAt
        ? lead.updatedAt
        : (typeof lead.lastContactAt === 'string' && lead.lastContactAt ? lead.lastContactAt : new Date().toISOString()),
      interactions: Array.isArray(lead.interactions) ? lead.interactions : [],
      calls: Array.isArray(lead.calls) && window.OGCallModel
        ? lead.calls.map(window.OGCallModel.normalizeCall)
        : []
    };
  }
  ...
  function unionById(listA = [], listB = [], idKey = 'id') {
    const map = new Map();
  ...
  function pickScalarField(newer, older, key) {
    const n = newer?.[key];
    const o = older?.[key];
  ...
  /** Merge campo a campo (nível 2) entre dois leads com o mesmo id. */
  function mergeLeadRecords(a, b) {
    const aT = leadTimestamp(a);
    const bT = leadTimestamp(b);
    ...
    const scalarKeys = [
      ...
      'observacoes', 'createdDate'
    ];
    const merged = { ...older, ...newer };
    for (const key of scalarKeys) {
      merged[key] = pickScalarField(newer, older, key);
    }
    const newerFleet = Number(newer.fleetSize);
    const olderFleet = Number(older.fleetSize);
    merged.fleetSize = Number.isFinite(newerFleet) && newerFleet > 0
      ? newerFleet
      : (Number.isFinite(olderFleet) ? olderFleet : 0);
    ...
    merged.calls = unionById(a.calls || [], b.calls || [], 'id')
      .map(call => window.OGCallModel ? window.OGCallModel.normalizeCall(call) : call)
  ...
  function mergeLeadLists(localLeads = [], remoteLeads = []) {
    const map = new Map();
    for (const lead of remoteLeads) {
      if (lead?.id != null) map.set(String(lead.id), normalizeLead(lead));
    }
    for (const lead of localLeads) {
      if (lead?.id == null) continue;
      const key = String(lead.id);
      const normalized = normalizeLead(lead);
      map.set(key, map.has(key) ? mergeLeadRecords(map.get(key), normalized) : normalized);
    }
    return [...map.values()];
  }

  function mergeHistoryLists(localHistory = [], remoteHistory = []) {
    return unionById(localHistory, remoteHistory, 'id')
      .sort((a, b) => Date.parse(b.date || 0) - Date.parse(a.date || 0))
      .slice(0, 5000);
  }

  function applyMergedState(leads, history) {
    state.leads = (leads || []).map(normalizeLead);
  ...
  function buildCommercialPanel(lead) {
    ...
    return `
      <section class="border-t border-slate-800 pt-4 og-commercial-form" aria-label="Contexto comercial do cliente">
        ...
        <div id="call-ai-panel"></div>
      </section>`;
  }

  function saveReviewedCall(lead, draft) {
    const call = window.OGCallModel.normalizeCall({
      ...draft,
      leadId: lead.id,
    ...
    const existingIndex = lead.calls.findIndex(item => item.id === call.id);
    if (existingIndex >= 0) lead.calls[existingIndex] = call;
    else lead.calls.push(call);

    const confirmedFields = [
      ['pain', call.pains.join('; ')],
      ['decisionMaker', call.decisionMaker],
      ['fleetSize', call.fleetSize],
      ['nextAction', call.nextAction],
      ['followUpAt', call.followUpAt]
    ];
    confirmedFields.forEach(([key, value]) => {
      if (value === '' || value === 0) return;
      if (lead[key] && String(lead[key]) !== String(value)) {
        if (!window.confirm(`Substituir "${key}" existente por a informação desta ligação?`)) return;
      }
      lead[key] = value;
    });
    ...
    lead.interactions.push({
      ...
      at: now,
      type: 'call',
      note: `${call.summary || 'Ligação registrada'}${call.analysisMode === 'heuristic-local' ? ' (rascunho heurístico revisado)' : ''}`
    });
    ...
    renderLeadsTable();
    renderLeadInspector();
    showNotification('Ligação salva e CRM atualizado.', 'success');
  }
  ...
  function renderLeadInspector() {
    ...
    window.OGCallUI?.render(lead, {
      save: draft => saveReviewedCall(lead, draft),
  ...
  function saveLeadsToStorage() {
    try {
      ...
      // Marca updatedAt no lead que acabou de ser tocado (selected)
      if (state.selectedLeadId) {
        const target = state.leads.find(l => l.id === state.selectedLeadId);
  ...
  async function initUnifiedExperience() {
    ...
    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.register('/service-worker.js').then(reg => {
        // Nova versão do SW em waiting → ativa na próxima visita ou após idle curto
      ...
      // Garante que a página use o SW novo após skipWaiting
      let refreshing = false;
      navigator.serviceWorker.addEventListener('controllerchange', () => {
        if (refreshing) return;
        refreshing = true;
        // Evita reload agressivo no primeiro install
        if (sessionStorage.getItem('og-sw-had-controller') === '1') {
          window.location.reload();
        }
        sessionStorage.setItem('og-sw-had-controller', '1');
      });
    ...
    // Toque no badge = forçar sync (ou reenfileirar se offline)
    const syncBadge = document.getElementById('og-sync-status');
    syncBadge?.addEventListener('click', () => {
      syncNow('manual');
    });

    // SW avisa quando Background Sync concluiu (Chrome/Edge)
    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.addEventListener('message', event => {
        if (event.data?.type === 'OG_SYNC_COMPLETE') {
          setSyncStatus('Sincronizado', 'ok');
          loadSharedState();
        }
      });
    ...
    // A cada 2 min online: descarrega outbox se houver pendência
    setInterval(async () => {
      if (!navigator.onLine) return;
      const pending = await readSyncOutbox();
      if (pending) await syncNow('interval');
    }, 120000);
    ...
    // Abertura: offline → trabalha só local; online → puxa servidor
    if (!navigator.onLine) {
      setSyncStatus('Modo offline — salvo neste aparelho', 'offline');
    } else {
      loadSharedState();
